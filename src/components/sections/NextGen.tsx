    ...
    <section ref={sectionRef} className="relative bg-navy-deep min-h-screen">
      ...
      <div className="px-4 md:px-6 pb-24 md:pb-32">
        <div className="max-w-[1000px] mx-auto">
          ...
          <div ref={exitRef} className="relative pl-10 md:pl-14">
            {/* Vertical line */}
            <div className="absolute left-[15px] md:left-[23px] top-0 bottom-0 w-px bg-white/10" />

            {EXIT_KEYS.map((key, i) => (
              <div key={key} className="exit-step relative mb-8 last:mb-0">
                {/* Dot */}
                <div className="absolute -left-10 md:-left-14 top-2 w-6 h-6 rounded-full bg-white/5 border-2 border-white/20 flex items-center justify-center">
                  <span className="font-sans text-[10px] font-bold text-white/60">{i + 1}</span>
                </div>
      ...
      <div className="px-4 md:px-6 pb-24 md:pb-32">
        <div className="max-w-[1280px] mx-auto">
          ...
          <div ref={deepRef} className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {DEEP_KEYS.map((key) => (
              <div
                key={key}
                className="deep-card p-7 md:p-8 rounded-xl bg-white/[0.04] border border-white/[0.08] hover:border-gold/20 transition-all duration-300"
              >
                <h3 className="font-serif text-[17px] md:text-[19px] font-semibold text-white leading-[1.2] mb-3">
                  {t(`deepDives.${key}.title`)}
                </h3>
                <div className="h-[1.5px] w-8 bg-gold/30 mb-4" />
                <p className="font-sans text-[13px] font-light leading-[1.7] text-white/60 mb-5">
                  {t(`deepDives.${key}.desc`)}
                </p>
                <span className="inline-block font-sans text-[10px] font-semibold tracking-wider uppercase text-gold/50 border border-gold/15 rounded-full px-3 py-1">
                  {t(`deepDives.${key}.freq`)}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="px-4 md:px-6 pb-24 md:pb-[140px]">
        <div className="max-w-[1280px] mx-auto">
          ...
          <div ref={partnerRef} className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-16">
            {/* NovaTalent */}
            <div className="relative p-8 rounded-xl overflow-hidden">
              <div className="absolute inset-0">
                <Image src="/images/gennext.jpg" alt="" fill className="object-cover" quality={60} loading="lazy" />
                <div className="absolute inset-0 bg-navy-deep/85" />
              </div>
              <div className="relative z-10">
                <h3 className="font-serif text-[22px] md:text-[26px] font-semibold text-gold leading-[1.2] mb-3">
                  {t('partnership.nova.title')}
                </h3>
                <p className="font-sans text-[14px] font-light leading-[1.7] text-white/65">
                  {t('partnership.nova.desc')}
                </p>
              </div>
            </div>
            ...
            <div className="relative p-8 rounded-xl overflow-hidden">
              <div className="absolute inset-0">
                <Image src="/images/img5.webp" alt="" fill className="object-cover" quality={60} loading="lazy" />
                <div className="absolute inset-0 bg-navy-deep/85" />
              </div>
              <div className="relative z-10">
                <h3 className="font-serif text-[22px] md:text-[26px] font-semibold text-gold leading-[1.2] mb-3">
                  {t('partnership.uni.title')}
                </h3>
                <p className="font-sans text-[14px] font-light leading-[1.7] text-white/65">
                  {t('partnership.uni.desc')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="text-center">
            <Link
              href="/contatti"
              className="inline-block font-sans text-[13px] md:text-[14px] font-semibold tracking-wider uppercase text-gold border-2 border-gold/40 rounded-full px-8 py-4 hover:bg-gold hover:text-navy-deep transition-all duration-300"
            >
              {t('cta')}
            </Link>
          </div>
